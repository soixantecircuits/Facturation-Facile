...
function display_operations(xmlstring) {
  ...
  $(xmlstring).find('tableau_operations').each(function() {
...
function save_edited() {
  $("#edited").each(function() {
    ...
    operationType = $("#edited").attr("name");
    operationValue = $("#edited").val();
...
function focus_element(element) {
  ...
  if (element.hasClass("categorie") || element.hasClass("compte")) {
    ...
    edit_input = $("#" + type + "_new").parent().html();
    element.html(edit_input);
    element.children(".new_operation").attr("id", 'edited').attr("class", 'categorie');
    if (type == "categorie") $("#edited").val(selectedType);
    else $("#edited").val(selectedType).width("60px");
  ...
  } else if (element.hasClass("provenance") || element.hasClass("objet") || element.hasClass("debit") || element.hasClass("credit") || element.hasClass("credit_tva") || element.hasClass("debit_tva") || element.hasClass("remarques")) // if the doubleclicked element is an input
  {
    ...
    edit_input = $('<input id="edited" type="text" name="' + element.attr("class").split(' ').slice(0, 1) + '" value="' + element.html() + '"/>');
...
$(document).ready(function() {
  ...
  $(document).keypress(function(e) {
    switch (e.keyCode) {
    case 13:
      // Key Enter : save current edited input
      save_edited();
      break;
    case 9:
      // Key Tab : save current edited input and focus on next element
      save_edited();
